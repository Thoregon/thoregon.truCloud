...
class UnifiedFileDescriptorMeta extends MetaClass {
    ...
    initiateInstance() {
        ...
        this.text("content", { description: 'Mime Type of the file', merge: false, emergent: true });
...
export default class UnifiedFileDescriptor extends ThoregonEntity() {
    ...
    getAvailableSource() {
        let source = this.sources.find((item) => item.isAvailable());
        if (!source && this.cid && this.name) {
            // assume we can rebuild the resourcesink source
            source = FileSource.recreateSource(this.cid, { filename: this.name, mimetype: this.mimetype });
