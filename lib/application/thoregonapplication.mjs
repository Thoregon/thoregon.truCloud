...
import { ErrAppsForSSInotAvailable } from "../errors.mjs";
...
export default class ThoregonApplication {
    ...
    async selectInstance() {
        ...
        if (!withUser) {
            ...
            if (startId) {
                startApp = await this.getAppInstance(appinstances, startId); // appinstances[startId];
    ...
    async getApps(retry = 3){
        const apps = await me.apps;
        if (!apps) {
            if (retry < 0) throw ErrAppsForSSInotAvailable(retry);
            await timeout(500);
            return this.getApps(--retry);
        }
        return apps;
    }

    async getApp(apps, id, retry = 3){
        const app = await apps[id];
        if (!app) {
            if (retry < 0) throw ErrAppsForSSInotAvailable(retry);
            try { apps.__requestSync__() } catch (ignore) {}
            await timeout(500);
            return this.getApp(apps, id, --retry);
        }
        return app;
    }

    async getAppInstance(app, id, retry = 3){
        const instance = await app[id];
        if (!instance) {
            if (retry < 0) throw ErrAppsForSSInotAvailable(retry);
            try { app.__requestSync__() } catch (ignore) {}
            await timeout(500);
            return this.getAppInstance(app, id, --retry);
        }
        return instance;
    }
