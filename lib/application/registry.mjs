...
export default class Registry extends Reporter(EventEmitter) {
     ...
     * @param {String}      domain
     */
    addDomain(appname, domain) {
        let appentry = this.apps[appname];
        if (!appentry) throw ErrAppUnknown(appname);
        let domains = appentry.domains;
        if (!domains) {
            domains = [];
            appentry.domains = domains;
        }
        domains.push(domain);
        return this;
    }
    ...
    addQuery(appname, name, resolver) {
        ...
        queries[name] =  Query.at(resolver);
        return this;
    }

    addEntitySchema(appname, name, schema) {
        let entry = this.apps[appname];
        if (!entry) {
            entry = { schemas: {}, appCls: null, uiCls: null, app: null, ui: null, uiroot: null };
            this.apps[appname] = entry;
        }
