...
export default class URLRouter extends Reporter() {
    ...
    refFromQuery(urlstr) {
        ...
        let appparam = params.get('app') || params.get('widget');
        return appparam ? `e=app,cn=${appparam}` : undefined;
    }

    refFromUrl(urlstr) {
        let url = new URL(urlstr);
        let hostname = url.hostname;
        return hostname && hostname !== 'localhost' ? `e=app,cn=${hostname}` : undefined;
    }
