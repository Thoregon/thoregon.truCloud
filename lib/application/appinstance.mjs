...
export class AppInstanceMeta extends MetaClass {
    ...
    initiateInstance() {
        ...
        this.collection("services",     Directory, { autocomplete: false, autoinit: false, description: 'services for the app' });
...
*/
    ...
    get home() {
        if (!this._home) {
            const Home = this._Home ?? this._app?.homeConstructor ?? globalThis.app?.homeConstructor;
            if (Home) {
...
//        if (!Home || this._home) return;
    ...
    get services() {
        if (!this._services) this._services = Directory.create();
