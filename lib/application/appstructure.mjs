...
*/
    ...
    async prepare(href) {
        ...
        try {
            ...
            // just log
            if (thoregon.debug) this.logger.error(ignore);
        }
        // component exports
        try {
        ...
        if (this.commandentries) {
            await this.commandentries.entries.aForEach(async (entry) => {
                try {
                    ...
                    if (thoregon.debug) this.logger.error("Can't load Command", e);
                }
            })
        }
        ...
        if (this.querieentries) {
            await this.querieentries.entries.aForEach(async (entry) => {
                try {
                    ...
                    if (thoregon.debug) this.logger.error("Can't load Query", e);
                }
            })
        }
        ...
        if (this.componentsentries) {
            await this.componentsentries.entries.aForEach(async (entry) => {
                try {
                    ...
                    if (thoregon.debug) this.logger.error("Can't load custom element", e);
                }
            })
        }
    ...
    async requestTestData(ctx) {
        ...
        ctx._$_addTempGalaxies(galaxies);
    }
