...
import ComponentDescriptor       from "./componentcontext.mjs";
// import ThoregonApplication       from "./thoregonapplication.mjs";
import ThoregonRemoteApplication from "./thoregonremoteapplication.mjs";
...
let AppClass = ThoregonRemoteApplication;
...
export default class AppStructure extends Reporter() {
    ...
    static async with(componentname, href, appstruct) {
        ...
        const proxy = null; // await ResourceProxy.on(href);

        const appstructure = new this(descriptor, proxy);
        await appstructure.prepareAppCtx(appstruct, href);
        return appstructure;
    }
    ...
    async appElementBase() {
        if (!AppElement) AppElement = universe.AppElement; // (await import("/thoregon.aurora/lib/blueprint/appelement.mjs")).default;
        return AppElement;
    }

    async auroraElement() {
        if (!AuroraElement) AuroraElement = universe.AuroraElement; // (await import("/thoregon.aurora/lib/auroraelement.mjs")).default;
        return AuroraElement;
    }

    async prepareAppCtx(appstruct, href) {
        const base        = this.base;
        const App         = appstruct.App;
        const translation = appstruct.i18n[this.getCurrentLanguage()];
        let ctx           = universe.dorifer.context(this.descriptor.id);
        let app           = await this.instanciate(App, href);     // new AppClass(this.descriptor.id);
        let ui            = await this.buildAppElement(this.descriptor.id, base + this.struct.ui.path, app);

        ctx
            .addAppDefault(app, ui)
            .withStructure(this);

        app.setViewMap(appstruct.ui);
        this.routes = appstruct.routes;
        ...
        const components = Object.entries(appstruct.components ?? {});
        const AuroraElement = await this.auroraElement();
        for (const [name, appelement] of components) {
    ...
    async prepare(href) {
        ...
        // now register
        let ctx = universe.dorifer.context(this.descriptor.id);
        let app = await this.appInstance(href);     // new AppClass(this.descriptor.id);
        let ui;
...
*/
        ...
        if (thoregon.ui && this.componentsentries) {
            ...
            for await (const componentpath of this.componentsentries) {
                try {
                    const name = componentpath.substring(componentpath.lastIndexOf('/') + 1);
                    const module = await this.import(base  + componentpath + '/' + name + '.mjs');
                    const appelement  = module.default;
...
*/
    ...
    async appInstance(href) {
        ...
        try {
            ...
            app = new AppClass(this.descriptor.id);
