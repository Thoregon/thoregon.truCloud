...
export default class AppStructure extends Reporter() {
    ...
    static async from(componentname, href) {
        ...
        let proxy;
        // if (universe.DEV) {
            proxy = await ResourceProxy.on(href);
...
*/
    ...
    async prepare(href) {
        ...
        if (this.i18entries) {
            ...
            const entry = this.i18entries.entries.find(entry => entry.startsWith(language));
            if (entry) {
...
*/
    ...
    async appInstance(href) {
        ...
        try {
            App = (await this.import(`${href}/${appmodule}`)).default;
            const app = new App(this.descriptor.id);
        ...
        const app = new ThoregonApplication(this.descriptor.id);
        universe.global('app', app);
        return app;
    }
