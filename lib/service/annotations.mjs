...
import ChannelController            from "/thoregon.archetim/lib/localmq/channelcontroller.mjs";
    ...
    serve(target, handle, appinstance, home) {
        setTimeout( () => {
            Object.entries(this._methods).forEach(([mthname, handler]) => {
                handler.attachTo(target, handle, appinstance, home, { callLimit: this.params?.callLimit, interval: this.params?.interval });
            })
        }, 500);
    }
...
class OnMessageHandler {
    ...
    attachTo(target, handle, appinstance, home, opt) {
        ...
        (async () => {
            const history          = await agent.current.getHistory(servicename, channelname);
            this.channelController = ChannelController.with(channel, history, async (evt) => {
    ...
    _attachTo(target, handle, appinstance, home, opt) {
        console.log("OnMessageHandler attachTo()");
        let fn = target[this.mthname];
        if (!fn) {
            console.log("OnMessage: Method does not exist", this.mthname);
            return;
        ...
        fn            = fn.bind(target);
        const channel = home.channels[this.channel];
        if (!channel) {
            console.log("OnMessage: Channel does not exist", this.channel);
            return;
        }
        const servicename      = target.constructor.name;
        const history          = this._getHistory(handle, servicename);
        this.channelController = ChannelController.with(channel, history, async (evt) => {
            if (evt.type !== this.event) return;
            // todo: check all conditions
            for await (const conditionfn of this.conditions) {
    ...
    _getHistory(handle, servicename) {
        const histories = this._getHistories(handle);
        let history = histories[this.channel];
        if (!history) history = histories[this.channel] = ChannelHistory.create({ servicename });
        if (!history.servicename) history.servicename = servicename;
        return history;
    }

    _getHistories(handle) {
        let histories = handle.histories;
        if (histories == undefined) histories = handle.histories = Directory.create();
        return histories;
    }
