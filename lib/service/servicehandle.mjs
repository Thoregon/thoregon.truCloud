...
export default class ServiceHandle extends EntityResolver(ThoregonEntity()) {
    ...
    //
    async consumer() {
        const producers = agent.current._producers;
        const service = Object.values(producers).find((producer) => producer.handle === this);
        return service;
    }

    async __consumer(retry = 3) {
        if (this._consumer) return this._consumer;
        const srvroot  = await this.source;
        if (!srvroot) return;   // can't request a queue
        try {
            const MQ       = universe.mq;
            const consumer = await MQ.getService(srvroot);
            // const consumer = await Facade.use(await NeulandConsumer.from(srvroot));
            // this._consumer = consumer;
            return consumer;
        } catch (e) {
            await timeout(300);
            return (--retry) < 1 ? undefined : this.consumer(retry);
        }
    }
