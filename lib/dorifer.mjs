...
import * as business         from "/thoregon.telom/lib/math/business.mjs";
...
export default class Dorifer extends Reporter(EventEmitter) {
    ...
    async init() {
        ...
        if (thoregon.deviceInfo?.agent) await this.awakeAgent();
...
*/
    ...
    checkinClass(origin, cls, metaClass) {
        ...
        this.class2origin.set(cls, { repo: ref, metaClass, origin });
    ...
    moduleUrl4Origin(origin) {
        if (!origin.startsWith('repo:')) return;
        let url = origin.substring(5);
        let i = url.indexOf(':');
        if (i > -1) url = url.substring(0,i);
        return url;
    }

    export4origin(origin) {
        let i = origin.lastIndexOf(':');
        if (i > -1) return  origin.substring(i+1);
    }
    ...
    cls4origin(origin) {
        ...
        let Cls = this.origin2class.get(origin)?.class;
        if (!Cls) {
            const i = origin.indexOf('@');
            if (i > -1) {
                const j = origin.indexOf('/', i);
                if (j > -1) origin = origin.substring(0, i) + origin.substring(j);
            } else {
                try {
                    origin = origin.substring(0, origin.indexOf('/', 6)) + '@1.0.0' + origin.substring(origin.indexOf('/', 6));
                } catch (e) {
                }
            }
            Cls = this.origin2class.get(origin)?.class;
        }
        return Cls
    }

    async fetchCls4origin(origin) {
        try {
            const url = this.moduleUrl4Origin(origin);
            if (!url) return Object;
            await import(url);
            // if nor exception the class must now be available in dorifers registry
            return this.cls4origin(origin);
        } catch (e) {
            console.log('load class for origin', origin, e);
            return Object;
        }
    }
...
*/
    ...
    async findInRepo(appname) {
        try {
            ...
            // const href = `/${appname}`;
            const href = `/${appname}`;
            let module = await fetch(`/${appname}/app.mjs`);
...
*/
    ...
    async activateApp(appname) {
        let { instance } = await this.findMatchingApp(appname);
        return instance;
    }
