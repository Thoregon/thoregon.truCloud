...
async function onattach(service, appinstence) {
    await service.attach?.(appinstence);
}
...
*/
    ...
    async setup(specs) {
        ...
        // init producers
        for await (const { name, producer, consumer, spec, appinstance, hooks } of producers) {
            const settings = spec.settings;
            try {
                // add schedulers if required
                if (create.includes(name)) {
                    await hooks.oninstall(consumer, settings);
                    if (appinstance) await hooks.onattach(consumer, appinstance);
                }
