...
async function onattach(service, handle, appinstence) {
    await service.attach?.(handle, appinstence);
}
...
*/
    ...
    async setup(specs) {
        ...
        // init producers
        for await (const { name, producer, consumer, spec, handle, appinstance, hooks } of producers) {
            const settings = spec.settings;
            try {
                const home = await appinstance.home;
                if (!home && spec.home) appinstance.home = new spec.home(appinstance);
                ...
                await hooks.onattach(producer, handle, appinstance);
                if (decorated !== producer) await onattach(decorated, handle, appinstance);
