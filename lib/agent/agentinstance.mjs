...
//        setTimeout(() => this._keepAppSync_(appinstance), APP_SYNC_INTERVAL);
    ...
    async applyServices(specs) {
        ...
        await this.activateServices(allservices, specs.services, agentservices);
    ...
    async activateServices(names, specs, services) {
        for await (const [name, serviceHandle] of services) {
            if (!serviceHandle.active) return;
            if (!specs[name]) return;
            await this.activateService(name, specs[name], serviceHandle);
        }
