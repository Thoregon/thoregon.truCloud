...
// import Channel        from "/thoregon.archetim/lib/lightmq/channel.mjs";
import ChannelHistory from "/thoregon.archetim/lib/lightmq/channelhistory.mjs";
...
import scheduler      from "/thoregon.truCloud/lib/scheduler/scheduler.mjs";
// const scheduler = null;

// import { isClass }    from "/evolux.util";

const Channel = universe.CHANNELS_SQLITE
                 ? (await import('/thoregon.archetim/lib/lightmq/channelsqlite.mjs')).default
                 : (await import('/thoregon.archetim/lib/lightmq/channel.mjs')).default;
...
*/
    ...
    async applyChannels(specs) {
        ...
        for await (let [name, spec] of entries) {
            ...
            if (!channel) {
                channel = await this.createChannel(name, spec);
                await channel.store();
            }
    ...
    async loadChannel(name) {
        ...
        if (!await channel.start()) {
            throw new Error(`Channel can not be started: ${name}`);
        }
        return channel;
    }

    async createChannel(name, spec) {
    ...
    async getHistory(servicename, channelname, methodname) {
        const name = ChannelHistory.getDBEntryName(servicename, channelname, methodname);
        let history = this._histories[name];
        if (history) {
            return history;
        }
        this._histories[name] = history = await this.loadHistory(servicename, channelname, methodname);
        if (history) {
            return history;
        }
        this._histories[name] = history = this.createHistory(servicename, channelname, methodname);
        await history.store();

        return history;
    }

    async loadHistory(servicename, channelname, methodname) {
        const history = await ChannelHistory.load(servicename, channelname, methodname);
        return history;
    }

    createHistory(servicename, channelname, methodname) {
        const history = ChannelHistory.forChannel({ servicename, channelname, methodname });
        return history;
    }
    ...
    async applyApp(specs) {
        ...
        try {
            ...
            await this.applyWhenAppinstanceAvailable(specs);
            return true;
        } catch (e) {
    ...
    async applyWhenAppinstanceAvailable(specs) {
        const apps         = me.apps;
        ...
        if (appinstance == undefined) {
            if (appSpec.create) {
                ...
                await this.restartApp(appinstance, instid, appid, specs);
            } else {
    ...
    async restartApp(appinstance, instid, appid, specs) {
        if (appinstance.id !== instid) appinstance.id = instid;
        if (appinstance.appid !== appid) appinstance.appid = appid;
        if (!appinstance._Home) appinstance._Home = this._Home;
        appinstance.restart();
        app.current = appinstance;
        this._keepAppSync_(appinstance);
        await this.applyServices(specs);
        console.log("-- AgentInstance: app restarted");
        await appinstance.home.doInitializationsAfterStart();
    }
...
//        setTimeout(() => this._keepAppSync_(appinstance), APP_SYNC_INTERVAL);
    ...
    applySpecs(specs) {
        ...
        console.log(DBGID, "apply service specs");
    ...
    async setup(specs) {
        console.log(DBGID, "setup");
        await this.init();

        await this.applyWWW(specs);
        return await this.applyApp(specs);
    }
    ...
    async startSchedules(setupSchedules) {
        if (!scheduler) return;
        if (setupSchedules.length === 0) return;
        const app = globalThis.app?.current
        const home = app.home;
        this._scheduler = scheduler;

        try {
            scheduler.useHome(home);

            for await (const setupSchedule of (setupSchedules ?? [])) {
                await setupSchedule(scheduler, home);
            }

            await scheduler.init();
            await scheduler.runMissedTasks();
        } catch (e) {
            console.error(">> AgentInstance.startSchedules() ERROR", e);
        }
    }

    async stopSchedules() {
        if (!this._scheduler) return;   // no scheduler
    ...
    async applyServices(specs) {
        ...
        const activate     = servicenames; // currentnames.filter((name) => servicenames.includes(name));

        const allservices = [...activate];

        // this.createServices(create);
        await this.activateServices(allservices, specs.services);
        ...
        setTimeout(() => this.startSchedules(specs.schedules), 600);
    ...
    createServices(names) {
        ...
        names.forEach((servicename) => {
            ...
            console.log(DBGID, "create service", servicename);
    ...
    async activateServices(names, specs) {
        for await (const name of names) {
            if (!specs[name]) continue;
            await this.activateService(name, specs[name]);
        }
        console.log("-- AgentInstance: services activated");
    }
...
*/
    ...
    async activateService(name, spec) {
        try { // const producerref = serviceHandle.producerref;
            ...
            console.log(DBGID, "activate service DONE", name);
        } catch (e) {
            console.log(DBGID, "ERROR: activate service", name);
    ...
    async shutdown() {
        console.log("== AgentInstance: shutdown");
        ...
        for await (const [name, servicespec] of Object.entries(services)) {
            ...
            try {
                ...
                console.log("-- AgentInstance: Service shutdown", name);
            } catch (e) {
                console.error(">> AgentInstance.shutdown Service", e);
                this._errors.push(e);
            }
        }
        const channels = this._channels;
        for await (const [name, channel] of Object.entries(channels)) {
            try {
                await channel.stop();
                console.log("-- AgentInstance: Channel shutdown", name);
            } catch (e) {
                console.error(">> AgentInstance.shutdown Channel", e);
            }
        }
        const histories = this._histories;
        for await (const [name, history] of Object.entries(histories)) {
            try {
                await history.stop();
                console.log("-- AgentInstance: History shutdown", name);
            } catch (e) {
                console.error(">> AgentInstance.shutdown History", e);
            }
        }

        console.log("-- AgentInstance: NeulandDB shutdown");
        await universe.neuland?.stop();

        console.log("== AgentInstance: shutdown DONE");
        console.$flush();
    }
