...
*/
    ...
    applyChannels(specs) {
        ...
        if (!channels) return;
        Object.entries(channels).forEach(([name, spec]) => {
            // load or create local channels
            let channel = this.loadChannel(name);
            if (!channel) {
    ...
    getHistory(servicename, channelname) {
        ...
        this._histories[name] = history = this.loadHistory(servicename, channelname);
        if (history) {
            history._save = (history) => { this.storeHistory(history) };
            return history;
        }
    ...
    loadHistory(servicename, channelname) {
        const agentid = this.id;
        let name = `${servicename}.${channelname}`;
    ...
    async applyApp(specs) {
        ...
        try {
            ...
            const App = (await import(`/${id}/${appmodule}`)).default;
    ...
    applyWhenAppinstanceAvailable(specs) {
        ...
        const appinstance = apps?.[appSpec.id]?.[appSpec.instance];
        if (appinstance == undefined) {
            ...
            if (!app.id) app.is = appSpec.instance;
            appinstance.restart();
    ...
    async activateService(name, spec, serviceHandle) {
        try { // const producerref = serviceHandle.producerref;
            ...
            if (this._Home) appinstance.useHome(this._Home);
            ...
            const decorated = this.decoratedProducer(producer, spec);
