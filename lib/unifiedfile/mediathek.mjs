...
export default class Mediathek {
    ...
    async add(content, { filename, mimetype, storage, credentials, mediathek = true, tags, timeout, onProgress = () => {} } = {}, abortSignal) {
        ...
        const store = this.storageAdapter(storage, { filename, mimetype, storage, credentials, mediathek, tags, timeout, onProgress });     // get the storage adapter
        ...
        let cid     = await store.cid(content, { filename, mimetype, storage, credentials, mediathek, tags, timeout, onProgress });         // get the content id for the content
        ...
        ufd = ufd ?? await UnifiedFileDescriptor.create({ cid, content: store.contentForUnifiedFileDescriptor(content), name: filename, mimetype, mediathek, tags: tags == undefined ? '' : Array.isArray(tags) ? tags.join(',') : tags.toString() });

        // create and attach a source
        let source = await store.createSource(cid, { filename, mimetype, credentials, content, use: true, mediathek });
        if (source) {
