...
export default class Mediathek {
    ...
    async add(content, { filename, mimetype, storage, credentials, mediathek = true, tags, timeout, onProgress = () => {} } = {}, abortSignal) {
        ...
        ufd = await UnifiedFileDescriptor.create({ cid, name: filename, mimetype, tags: tags == undefined ? '' : Array.isArray(tags) ? tags.join(',') : tags.toString() });

        // create and attach a source
        let source = await store.createSource(cid, { filename, mimetype, credentials, use: true, mediathek: this });
        if (source) {
    ...
    async addSource(cid, content, { filename, mimetype, storage, credentials, mediathek = true, tags, timeout, onProgress = () => {} } = {}, abortSignal) {
        // todo
    }
     ...
     */
    async maintainTags(cid, tags) {
        const mttags = await this.tags;
        for await (const tag of tags) {
            let tagcids = await mttags.get(tag);
            if (!tagcids) {
                tagcids = await Directory.create(); // todo [OPEN]: need persistent 'Set'
                await mttags.put(tag, tagcids);
            }
            tagcids.put(cid, 1);
        }
    }

    async removeTags(cid, tags) {
        const mttags = await this.tags;
        for await (const tag of tags) {
            let tagcids = await mttags.get(tag);
            if (!tagcids) continue;
            await tagcids.remove(cid);
        }
    }
