...
export default class Mediathek {
    ...
    async add(content, { filename, mimetype, storage, credentials, mediathek = true, tags, timeout, onProgress = () => {} } = {}, abortSignal) {
        ...
        ufd = ufd ?? await UnifiedFileDescriptor.create({ cid, content, name: filename, mimetype, mediathek, tags: tags == undefined ? '' : Array.isArray(tags) ? tags.join(',') : tags.toString() });
