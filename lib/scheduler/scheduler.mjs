...
class Scheduler {
    constructor() {
        ...
        FIND_SCHEDULES_SQL = universe.olapSQLite
                             ? `SELECT * FROM schedules WHERE DATETIME(next_run) <= DATETIME(?) AND (type <> 'repeating' OR executed = false)`
    ...
    async runMissedTasks() {
        try {
            ...
            const res    = await olap.query(FIND_SCHEDULES_SQL, [nowISO]);
            const rows   = res.rows;
    ...
    async _tick() {
        try {
            ...
            const nowISO = Temporal.Now.instant().toString();
            const res    = await olap.query(FIND_SCHEDULES_SQL, [nowISO]);
            const rows = olap.convertToObjects(res);
            for (const row of rows) {
    ...
    async _runJob(row, { isMissed }) {
        ...
        const scheduleid = row.scheduleid;      // scheduleid
        const function_id = row.function_id;
        const spayload = row.payload;
        const expression = row.expression;
        const retry_plan = row.retry_plan;
        const lastRunISO = Temporal.Now.instant().toString();
        let retriesLeft = row.retries_left ?? 0;   // retries_left
        let nextRunISO = null;
        ...
        if (!fn) {
            ...
            console.error(">> Scheduler.runJob():", errorMessage);
        } else {
            try {
...
class RepeatingBuilder {
    ...
    async schedule() {
        ...
        const res = await olap.query(
            `INSERT INTO schedules (name, type, function_id, expression, next_run, payload) VALUES(?, ?, ?, ?, ?, ?) RETURNING scheduleid`,
            [
                this._name,
                'repeating',
                this._functionId,
        ...
        const id = res?.rows?.[0].scheduleid;
        return id;
    }
}
...
class NextBuilder {
    ...
    async schedule() {
        ...
        const res = await olap.query(
            `INSERT INTO schedules (name, type, function_id, expression, next_run, payload, executed) VALUES(?, ?, ?, ?, ?, ?, 0) RETURNING scheduleid;`,
            [
                this._name,
                'next',
                this._functionId,
...
class RetryBuilder {
    ...
    async schedule() {
        ...
        const res = await olap.query(
            `INSERT INTO schedules (name, type, function_id, expression, next_run, payload, retries_left, retry_plan) VALUES(?, ?, ?, ?, ?, ?, ?, ?) RETURNING scheduleid`,
            [
                this._name,
                'retry',
                this._functionId,
