...
export default class Query {
    ...
    refresh() {
        ...
        if (this._retarded) {
            this._pendingQ.push(() => this._onFlush());
        }
        this._onFlush();
        this.accelerate();
    }
...
class ArrayQuery {
    ...
    prepare() {
        if (!this.listening) this.array.addEventListener?.('change', (evt) => this.changed(evt));
        this.listening = true;
    }

    changed(evt) {
        const { type, obj, property } = evt;
        this.retarded = true;
...
/*
        switch (type) {
            case 'delete':
                this.query.itemDropped(property);
                break;
            case 'change':
            case 'add':
                this.query.itemAdded(property, item);
                break;
        }
*/
    }
